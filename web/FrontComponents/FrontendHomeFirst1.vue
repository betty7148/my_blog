...
export default {
    ...
    methods:{
        async getHomeArticle(){
            console.log("111");
            this.pageIndex++;
            await this.$http.get("http://127.0.0.1:7001/home/article?pageIndex="+this.pageIndex+"&perPageNum="+this.perPageNum).then(async function(result){
                ...
                for(let i=0;i<this.articleList.length;i++){
                    ...
                    await that.$store.dispatch('getCommentNum',{id:this.articleList[i].id,that:that});
                    this.articleList[i].comment_num=that.$store.state.commentList[this.articleList[i].id];
                    console.log(this.articleList[i].id+"获取到了"+this.articleList[i].comment_num);
                }
                // this.articleList.forEach(async function(item){
